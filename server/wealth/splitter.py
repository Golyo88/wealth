...
def extract_representative_name(page_text):
    """
    Megpróbálja kinyerni a képviselő nevét a szövegből.
    Feltételezzük, hogy a név a "A nyilatkozatot adó neve:" kulcsszó után szerepel.
    """
    match = re.search(r"A nyilatkozatot adó neve:\s*(.+)", page_text)
    if match:
        # Az első sorban szereplő szöveget vesszük a névnek.
        name_line = match.group(1).strip().splitlines()[0]
        # Fájlnév-barát formátum: eltávolítja az érvénytelen karaktereket.
...
def split_pdf_by_representative(pdf_path, output_dir):
    ...
    # Minden szakaszt külön PDF fájlba mentünk.
    for section in representative_sections:
        writer = PdfWriter()
        for i in range(section["start"], section["end"] + 1):
            writer.add_page(reader.pages[i])
        output_filename = os.path.join(output_dir, f"{section['name']}-{section['start']}-{section['end']}.pdf")
...
def main():
    pdf_path = "Kepviselok_dec_20230712.pdf"  # Eredeti PDF fájl
    output_dir = "split_representatives"  # Kimeneti mappa
